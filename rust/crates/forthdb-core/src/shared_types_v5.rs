...
const SHARD_GROUP_SIZE: usize = 64;
...
type LocalMap<K, V> = PersistentHashMap<K, V, FnvBuildHasher>;
type RecordSet = PersistentHashSet<RecordId>;
type Shard<K, V> = Arc<LocalMap<K, V>>;
type ShardGroup<K, V> = Arc<Vec<Option<Shard<K, V>>>>;

#[derive(Clone)]
struct ShardedMap<K, V, const SHARDS: usize> {
    groups: Arc<Vec<Option<ShardGroup<K, V>>>>,
    len: usize,
}
...
impl<K, V, const SHARDS: usize> Default for ShardedMap<K, V, SHARDS> {
    fn default() -> Self {
        ...
        assert!(SHARDS >= SHARD_GROUP_SIZE);
        assert_eq!(SHARDS % SHARD_GROUP_SIZE, 0);
        Self {
            groups: Arc::new(vec![None; SHARDS / SHARD_GROUP_SIZE]),
            len: 0,
        }
    }
}
    ...
    V: Clone,
{
    ...
    fn directory_indices(key: &K) -> (usize, usize) {
        let shard_index = Self::shard_index(key);
        (
            shard_index / SHARD_GROUP_SIZE,
            shard_index & (SHARD_GROUP_SIZE - 1),
        )
    }
    ...
    fn get(&self, key: &K) -> Option<&V> {
        ...
        self.groups[group_index]
            .as_ref()
            .and_then(|group| group[local_index].as_ref())
    ...
    fn insert(&mut self, key: K, value: V) -> Option<V> {
        let (group_index, local_index) = Self::directory_indices(&key);
        let root = Arc::make_mut(&mut self.groups);
        let group = root[group_index]
            .get_or_insert_with(|| Arc::new(vec![None; SHARD_GROUP_SIZE]));
        let group = Arc::make_mut(group);
        let shard = group[local_index]
    ...
    fn remove(&mut self, key: &K) -> Option<V> {
        ...
        if shard.is_empty() {
            group[local_index] = None;
        }
        if group.iter().all(Option::is_none) {
            root[group_index] = None;
        }
        previous
    }
