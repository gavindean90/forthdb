...
current `World` with `OnceLock`. History nodes retain only weak references to
live projected ancestors, so the authoritative history does not pin every old
query kernel in memory.
